...
{{- end }}`)
        ...
        expect(yaml).toContain('config: "{{- with .Values.database }}');
        expect(yaml).toContain('{{- end }}"');
    });
...
{{- end }}`)
        ...
        expect(yaml).toContain('services: "{{- range $name, $service := .Values.services }}');
        expect(yaml).toContain('{{- end }}"');
    });
...
{{- end }}`)
        ...
        expect(yaml).toContain('annotations: "{{- if eq .Values.cloud');
