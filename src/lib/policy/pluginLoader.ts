...
export class PluginLoader {
  ...
  async loadFromPackage(packageName: string, version?: string): Promise<PluginLoadingResult> {
    ...
    try {
      ...
      throw new PluginError(
        `Failed to load plugin from package '${packageName}': ${error instanceof Error ? error.message : UNKNOWN_ERROR_MESSAGE}`,
        packageName,
  ...
  async discoverPlugins(searchPattern?: string): Promise<PluginDiscoveryResult[]> {
    ...
    try {
      ...
      throw new PluginError(
        `Plugin discovery failed: ${error instanceof Error ? error.message : UNKNOWN_ERROR_MESSAGE}`,
        'discovery',
      );
    }
  }
    ...
    const loadPromises = packages.map(async (pkg) => {
      try {
        const [packageName, version] =
          pkg.includes('@') && !pkg.startsWith('@') ? pkg.split('@') : [pkg, undefined];
   ...
   */
  static validatePluginInterface(
    plugin: unknown,
    packageName: string,
  ): asserts plugin is PolicyPlugin {
   ...
   * @private
   */
  private shouldLoadInEnvironment(_plugin: PolicyPlugin): boolean {
