...
export function aggregateResults(results: PolicyResult[]): PolicyResult {
  ...
  for (const result of results) {
    ...
    if (result.metadata.startTime) {
      if (!earliestStartTime || result.metadata.startTime < earliestStartTime) {
...
export function generateResultSummary(
  ...
  for (const violation of allViolations) {
    violationsByPlugin[violation.plugin] = (violationsByPlugin[violation.plugin] || 0) + 1;
  }
...
export function groupViolationsByPlugin(
  ...
  for (const violation of violations) {
    if (!grouped[violation.plugin]) {
      grouped[violation.plugin] = [];
    }
    const pluginViolations = grouped[violation.plugin];
    if (pluginViolations) {
