...
function writeSingleAssetFile(
  ...
  const timonelHeader = '# Do not modify - Generated by Timonel\n';
  ...
  if (parts.length === 1) {
    ...
    // eslint-disable-next-line security/detect-non-literal-fs-filename -- Chart writer needs dynamic paths
    fs.writeFileSync(path.join(outDir, targetDir, filename), timonelHeader + parts[0] + '\n');
  } else {
    // Multiple documents - use numbered filenames
    parts.forEach((doc, index) => {
      const filename = `${assetId}-${index + 1}.yaml`;
      // eslint-disable-next-line security/detect-non-literal-fs-filename -- Chart writer needs dynamic paths
      fs.writeFileSync(path.join(outDir, targetDir, filename), timonelHeader + doc + '\n');
    });
  }
}
